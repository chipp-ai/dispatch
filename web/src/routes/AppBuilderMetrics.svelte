<script lang="ts">
  ...
  onMount(async () => {
    ...
    await Promise.all([loadApp(), loadSessions()]);
  });
  ...
  async function loadSessions() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/chat/${params.appId}/sessions?limit=100`,
        { credentials: "include" }
      );
...
<svelte:head>
  <title>App Metrics - Chipp</title>
</svelte:head>

<div class="app-builder">
  <BuilderSidebar appId={params.appId} activeTab="metrics" />
